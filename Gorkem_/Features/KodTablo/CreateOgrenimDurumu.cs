...
namespace Gorkem_.Features.KodTablo
{
    public static class CreateOgrenimDurumu
    {
        ...
        internal sealed record Handler(GorkemDbContext Context, Serilog.ILogger Logger) : IRequestHandler<Command, Result<bool>>
        {
            ...
            {
                var isExists = Context.KT_OgrenimDurumus.Any(r => r.Name == request.Name);
                if (isExists) return await Result<bool>.FailAsync($"{request.Name} is already exists");

                Context.KT_OgrenimDurumus.Add(request.ToOgrenimDurum());
    ...
    public class CreateOgrenimDurumEnpoint : ICarterModule
    {
        ...
        {
            var mapGet=app.MapPost("kodtablo/ogrenimdurumu", async ([FromBody] OgrenimDurumuEkleRequest model, ISender sender) =>
            {
